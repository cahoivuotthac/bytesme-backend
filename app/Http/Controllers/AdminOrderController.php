			...
			->where(function ($query) {
				$query->where('order_payment_method', 'COD')
					->orWhere(function ($query) {
						$query->where('order_payment_method', 'Banking')
							->where('order_is_paid', 1);
					});
			})
			->with([
				'user:user_id,name',
	...
	public function updateOrderField(Request $request): JsonResponse
	{
		...
		try {
			if ($request->input('field') === 'order_status') {
				$allowedorder_statusValues = ['pending', 'delivering', 'delivered', 'cancelled'];
	...
	public function getStatistics(Request $request): JsonResponse
	{
		try {
			...
			return response()->json([
				...
				'order_statusData' => $order_statusCounts
			]);
		} catch (\Exception $e) {
